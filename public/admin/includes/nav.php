...
?>
<nav class="navbar navbar-expand-lg <?= $invNavLight ? 'navbar-light bg-light' : 'navbar-dark bg-dark' ?> admin-nav">
    <div class="container-fluid px-3 px-lg-4">
        ...
        <div class="collapse navbar-collapse" id="invAdminNavbar">
            <div class="d-flex flex-column flex-lg-row flex-wrap gap-2 ms-lg-auto align-items-stretch align-items-lg-center inv-nav-cluster py-3 py-lg-0">
                <?php $navBtn = $invNavLight ? 'btn-outline-dark' : 'btn-outline-light'; ?>
                <a class="btn <?= $navBtn ?> text-center text-lg-start<?= $isDashNav ? ' active' : '' ?>"
                ...
                <a class="btn <?= $navBtn ?> text-center text-lg-start<?= $isCompaniesNav ? ' active' : '' ?>"
                ...
                <a class="btn <?= $navBtn ?> text-center text-lg-start<?= $isTimeNav ? ' active' : '' ?>"
                ...
                <a class="btn <?= $navBtn ?> text-center text-lg-start<?= $isInvoicesNav ? ' active' : '' ?>"
                ...
                <div class="dropdown inv-nav-end-dropdown">
                    <button class="btn <?= $navBtn ?> text-center text-lg-start dropdown-toggle<?= $isSettingsNav ? ' active' : '' ?>"
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        <li><a class="dropdown-item" href="<?= htmlspecialchars(dsc_invoicing_href('admin/appearance.php'), ENT_QUOTES, 'UTF-8') ?>"><i class="bi bi-palette me-2" aria-hidden="true"></i>Appearance</a></li>
                ...
                <a class="btn <?= $navBtn ?> text-center text-lg-start<?= $isHelpNav ? ' active' : '' ?>"
                ...
                <?php if ($username !== ''): ?>
                    <span class="navbar-text <?= $invNavLight ? 'text-secondary' : 'text-white-50' ?> small px-lg-2 py-1 text-center text-lg-start"><?= htmlspecialchars($username, ENT_QUOTES, 'UTF-8') ?></span>
                <?php endif; ?>
                <form method="POST" action="<?= htmlspecialchars(dsc_invoicing_href('admin/logout.php'), ENT_QUOTES, 'UTF-8') ?>" class="m-0">
                    <?= csrfField() ?>
                    <button type="submit" class="btn <?= $navBtn ?>" aria-label="Log out">
